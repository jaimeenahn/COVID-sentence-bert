...
class TripleSoftmaxLoss(nn.Module):
    def __init__(self,
        ...
        self.hidden2output = nn.Linear(self.hidden, sentence_embedding_dimension)
    ...
    def forward(self, sentence_features: Iterable[Dict[str, Tensor]], labels: Tensor, document_rep: Tensor):
        ...
        print(rep_a.shape, rep_b.shape, document_rep.shape)
        vectors_concat = []
        ...
        if labels is not None:
            ...
            loss -= self.document_coef / 2 * torch.sum(torch.cosine_similarity(document_rep, rep_b)) # todo: MMI가 들어가면 좋긴하겠다.
            loss -= self.document_coef / 2 * torch.sum(torch.cosine_similarity(document_rep, rep_a))
            #print("Loss1\t", loss_fct(output, labels.view(-1)))
            print("Loss2_db\t", -torch.sum(torch.cosine_similarity(document_rep, rep_b)))
            print("Loss3_da\t", -torch.sum(torch.cosine_similarity(document_rep, rep_a)))
            '''
            return loss
        else:
            return reps, output
